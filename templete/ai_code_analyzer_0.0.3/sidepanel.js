...
console.log(Date.now(), "random Id", randomId);

// console.log("sidepanel random id", randomId)
  ...
  "success": true
        ...
        if (res.success) {
            ...
            if (res.data.status !== 2 && res.data.status !== 3) {
                ...
                console.log(Date.now(), "sidepanel plugine/repo_analyzer_check handle success", res)
                // 修改提示语
                ...
                if (res.data.status === 2) {
                    ...
                    isShow = true
                    updateLoad()
                    console.log(Date.now(), "sidepanel plugine/repo_analyzer_check finish success", "请求以处理完成", res)
                } else if (res.data.status === 3) {
                    let message = res.data.message;
                    updateError(message)
                    console.log(Date.now(), "sidepanel plugine/repo_analyzer_check finish error", "请求以处理失败", res)
                }
            }
        } else {
            updateError(res.data.message)
            console.log(Date.now(), "sidepanel plugine/repo_analyzer_check error", res.data.message)
        }
        ...
        console.log(Date.now(), "sidepanel plugine/repo_analyzer_check error", e)
    }
}
...
getAiContent = async () => {
    ...
    try {
        ...
        updateError("The back-end service is offline")
        console.log(Date.now(), "sidepanel plugine/repo_analyzer error", e)
    }
}
...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
        ...
        if (request.type === 'action_analyzer') {
            // 点击按钮的页面的tabId
            // messageTabid = request.tabid
            // 查询当前页面的tabId
            // chrome.runtime.sendMessage({type: "get-tabId", data: request.data})

            // let [tab] = await chrome.tabs.query({active: true, currentWindow: true})

            // messageTabid = request.tabid;  // 发消息的页面
            // console.log("侧边栏打印", tab.id, messageTabid)
            // if (tab.id == messageTabid) {
            console.log(Date.now(), "sidepanel analyzer", request)
            // console.log("触发消息通知")
            $("#sidepanel").css({
                "display": "block"
            })
            ...
            console.log(Date.now(), "random number", randomNumber);

            console.log("sidepanel to service-work,type is prepare_send_message", request.tabid)
            setTimeout(async () => {
                try {
                    let result = await chrome.storage.local.get("lock");
                    console.log("thread arrive", result["lock"])
                    if (result["lock"] === undefined) {
                        await chrome.storage.local.set({"lock": randomNumber})
                        //        判断哪个是
                        try {
                            let lockResult = await chrome.storage.local.get("lock");
                            if (lockResult["lock"] == randomNumber) {
                                ...
                                console.log("lock value is", lockResult["lock"], "current thread value is", randomNumber, "start request")
                                getAiContent()
                            }
                        } catch (e) {
                            console.log(e)
            ...
            setTimeout(async () => {
                ...
                console.log("sidepanle delete lock")
            }, 1500)
            ...
            // chrome.storage.sync.get("lock",(result)=>{
            //     if ()
            // })


            //     if (lockSidepanel === undefined) {

            // }
            // }, randomTime)
            // setTimeout(() => {
            //     lockSidepanel = undefined;
            // }, 1300)
        }
